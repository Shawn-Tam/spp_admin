...
export const useRouterStore = defineStore('router', () => {
  ...
  const SetAsyncRouter = async() => {
    ...
    for (let i = 0; i < asyncRouter.length; i++) {
      if (asyncRouter[i].ID === 23 || asyncRouter[i].ID === 25 || asyncRouter[i].ID === 2) {
        asyncRouter.splice(i, 1)
        i--
        continue
      }
      if (asyncRouter[i].children != null) {
        for (let j = 0; j < asyncRouter[i].children.length; j++) {
          if (asyncRouter[i].children[j].ID === 9 || asyncRouter[i].children[j].ID === 17 || asyncRouter[i].children[j].ID === 18 ||
              asyncRouter[i].children[j].ID === 20 || asyncRouter[i].children[j].ID === 21 || asyncRouter[i].children[j].ID === 22 ||
              asyncRouter[i].children[j].ID === 14 || asyncRouter[i].children[j].ID === 15 || asyncRouter[i].children[j].ID === 8 ||
              asyncRouter[i].children[j].ID === 5 || asyncRouter[i].children[j].ID === 6) {
            asyncRouter[i].children.splice(j, 1)
            j--
            console.log(j)
          }
        }
